...
echo "Question9"
awk '/\w+\s+\w+\s*\([^)]*\)/ {count++} END {print count}' code.c

# Question 10: Extract and list all unique function names using grep and cut
echo "Question10"
grep -Eo '^\s*\w+\s+(\w+)\s*\(' code.c | cut -d' ' -f2 | sort -u
